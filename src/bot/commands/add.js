    ...
    run: async function(client, message, args) {
        if (!message.channel.name.includes("ticket-")) {
            message.channel.send({
                embed: {
                    title: `**❌ | Hata**`,
                    description: `Bu bir bilet kanalı değil`,
                    color: 0xFF0000
                }
        ...
        if (!message.member.hasPermission('MANAGE_CHANNELS')) {
            message.channel.send({
                embed: {
                    title: `**❌ | Hata**`,
                    description: `bu komutu kullanmak için aynı izinlere ihtiyacınız var`,
                    color: 0xFF0000
                }
            }).then(async function(msg) {
                setTimeout(() => {
        ...
        if (!args[0]) {
            message.channel.send({
                embed: {
                    title: `**❌ | Hata**`,
                    description: `bilete katılmasını istediğiniz rolü/üyeyi belirtmeniz gerekiyor!`,
                    color: 0xFF0000
                }
            }).then(async function(msg) {
                setTimeout(() => {
